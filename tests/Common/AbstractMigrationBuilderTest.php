...
abstract class AbstractMigrationBuilderTest extends TestCase
{
    ...
    {
        $idType = 'integer';
        $primaryKey = $this->builder->primaryKey();

        if ($this->db->getDriverName() === 'oci') {
            $idType = 'decimal';
            $primaryKey = "NUMBER GENERATED by default on null as IDENTITY PRIMARY KEY";
        }

        $this->builder->createTable('test', ['id' => $primaryKey]);

        $schema = $this->db->getSchema()->getTableSchema('test');
        $this->assertNotEmpty($schema);
        $this->assertSame('id', $schema->getColumn('id')->getName());
        $this->assertSame($idType, $schema->getColumn('id')->getType());
